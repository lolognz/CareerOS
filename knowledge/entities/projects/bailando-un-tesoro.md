# Proyecto: Bailando un Tesoro

## Canonical Fields

| Field | Value |
|---|---|
| id | `project:bailando-un-tesoro` |
| name | Bailando un Tesoro |
| type | Videojuego educativo, producto cultural y proyecto de investigación |
| status | Publicado en Play Store y App Store; estado actual desconocido |
| chronology | Posterior a La Cortesía de España y dentro de la etapa UCM 2015–2019; fechas exactas no documentadas. |
| organization | Universidad Complutense de Madrid, en colaboración con el Ballet Nacional de España |
| organization_id | `company:ucm` |
| related_roles | [`role:ucm-research-software-development`] |
| context | Iniciativa para acercar la danza española a niños y públicos jóvenes mediante un videojuego. |
| problem | Representar la danza española de forma educativa y accesible, gestionando a la vez animaciones 3D largas y tiempos de carga adecuados para personas no habituadas a videojuegos. |
| contribution | Participación desde la definición de la idea hasta diseño, desarrollo, pruebas, presentación y publicación; interlocución técnica frecuente con el Ballet Nacional y coordinación con perfiles técnicos y artísticos. |
| systems | Videojuego 3D basado en una escena Unity que cambiaba escenarios, personajes, música y animaciones; conversaciones y animaciones procedentes de captura de movimiento. |
| technologies | [] |
| users_or_stakeholders | Niños, escuelas, Ballet Nacional de España, dirección del proyecto, equipo técnico, artistas 2D y 3D y colaboradores especializados. |
| outcomes | Publicación móvil; estudio con más de 700 niños; publicaciones; presencia en prensa, radio y televisión. El puesto número 2 y las 800.000 “reacciones” son cifras reportadas con reservas. |
| evidence | `knowledge/raw/interviews/2026-07-cto-adjunto-fast-track.md`; `knowledge/entities/roles/ucm-research-software-development.md` |
| uncertainty | No constan fechas exactas ni cifras verificadas de descargas. El significado de “reacciones” no está claro y el puesto número 2 se conserva como información reportada. |
| related_entities | [] |

## Contexto y contribución

La iniciativa surgió después de que el Ballet Nacional conociera La Cortesía de España. El usuario investigó la danza y los juegos musicales, participó en reuniones con la organización y ayudó a coordinar necesidades de representación cultural, jugabilidad, desarrollo y experimentación.

## Entidades candidatas pendientes

- Organización candidata: Ballet Nacional de España.
- Tecnología candidata: Unity3D.
- Tecnología candidata: C#.
- Tecnología candidata: captura de movimiento.
- Historia candidata: estudio con más de 700 niños.
- Historia candidata: entrevista de radio con el director del Ballet Nacional.
- Historia candidata: hackathon inicial de 48 horas.
- Publicaciones candidatas: artículos derivados del proyecto.

## Incertidumbre y evidencia

La evidencia no permite convertir la visibilidad reportada en una métrica de impacto educativo probado. Fuente principal: `knowledge/raw/interviews/2026-07-cto-adjunto-fast-track.md`.
