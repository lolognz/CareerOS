# Organización: Universidad Complutense de Madrid

## Canonical Fields

| Field | Value |
|---|---|
| id | `company:ucm` |
| name | Universidad Complutense de Madrid |
| aliases | [UCM] |
| industry | Educación superior e investigación |
| website | Desconocido en la evidencia disponible |
| countries | España |
| chronology | Etapa de investigación y desarrollo de software entre 2015 y 2019; las fechas de proyectos y contratos internos varían. |
| business-context | Contexto universitario de tecnología educativa, serious games, investigación aplicada, experimentos con usuarios, publicaciones y comunicación académica. |
| roles | [`role:ucm-research-software-development`] |
| projects | [`project:bailando-un-tesoro`, `project:comunicarte`, `project:la-cortesia-de-espana`] |
| stories | [`story:bailando-un-tesoro-user-tests`, `story:comunicarte-bbva-proposal`, `story:comunicarte-virtual-audience`, `story:la-cortesia-from-tfg-to-product`] |
| technologies | [`technology:c-sharp`, `technology:unity3d`] |
| skills | [`skill:interdisciplinary-collaboration`, `skill:requirements-analysis`, `skill:research-software-development`, `skill:stakeholder-communication`, `skill:user-testing-and-validation`] |
| achievements | [`achievement:bailando-un-tesoro-educational-games-ranking`, `achievement:bailando-un-tesoro-media-visibility`, `achievement:bailando-un-tesoro-user-testing`, `achievement:comunicarte-bbva-funding`, `achievement:la-cortesia-publication-and-release`, `achievement:la-cortesia-tfg-grade`] |
| evidence | `knowledge/raw/interviews/2026-07-cto-adjunto-fast-track.md`; `knowledge/entities/roles/ucm-research-software-development.md` |
| related-entities | [] |

## Contexto

La relación documentada agrupa varios proyectos vinculados a educación, artes escénicas, videojuegos e investigación: La Cortesía de España, Bailando un Tesoro y ComunicArte. Incluyó colaboración con escuelas, organizaciones culturales, Fundación BBVA y perfiles de distintas disciplinas. Estas organizaciones no se convierten aquí en relaciones canónicas porque sus entidades aún no existen.

## Entidades candidatas pendientes

- Organización relacionada candidata: Ballet Nacional de España.
- Organización relacionada candidata: Fundación BBVA.
- Publicaciones candidatas: publicaciones académicas derivadas de los tres proyectos, pendientes de identificación individual.

## Incertidumbre y fuentes

El intervalo 2015–2019 representa una secuencia de actividades y no confirma un contrato único y continuo. ComunicArte tuvo un contrato desde abril de 2018 y una duración prevista de 21 meses, pero no consta aquí su fecha exacta de cierre.

Fuentes: `knowledge/raw/interviews/2026-07-cto-adjunto-fast-track.md` y `knowledge/entities/roles/ucm-research-software-development.md`.
